const Notes = {
  ...
  removeMistake(id) { if(!confirm('Bu yanlış soru kaydı silinsin mi?')) return; DB.update({mistakes:DB.user().mistakes.filter(m=>m.id!==id)}); App.render(); App.toast('Soru kaydı silindi.'); },
